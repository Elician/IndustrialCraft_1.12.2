...
import ic2.core.item.upgrade.ItemUpgradeModule.UpgradeType;
...
public class BlocksItems {
  ...
  private static void initFluids() {
    Material steam = new MaterialLiquid(MapColor.field_151680_x);
  ...
  private static void initItems() {
    ...
    BlockDispenser.DISPENSE_BEHAVIOR_REGISTRY.func_82595_a(crafting, new BehaviorScrapboxDispense());
    ...
    new Tfbp();
    Item.ToolMaterial bronzeToolMaterial = EnumHelper.addToolMaterial("IC2_BRONZE", 2, 350, 6.0F, 2.0F, 13).setRepairItem(ItemName.ingot.getItemStack(IngotResourceType.bronze));
    ...
    new ItemDrill(ItemName.diamond_drill, 80, HarvestLevel.Diamond, 120000, 64, 1, 16.0F);
    new ItemDrill(ItemName.drill, 50, HarvestLevel.Iron, 80000, 192, 1, 8.0F);
    ...
    cell.addCapability(CapabilityFluidHandler.FLUID_HANDLER_ITEM_CAPABILITY, (stack) -> {
      ...
      if (type.isFluidContainer()) {
        cell.getClass();
        var10000 = new CellType.CellFluidHandler(stack, cell::getType);
      } else {
        var10000 = null;
      }
    ...
    upgrade.setRightClickHandler(UpgradeType.advanced_ejector, ItemHandlers.openAdvancedUpgradeGUI);
    upgrade.setRightClickHandler(UpgradeType.advanced_pulling, ItemHandlers.openAdvancedUpgradeGUI);
    new ItemTinCan();
  ...
  private static void registerIC2fluid(FluidName name, Material material, int color, int density, int viscosity, int luminosity, int temperature, boolean isGaseous) {
    ...
    if (!fluid.canBePlacedInWorld()) {
      ...
      fluid.setUnlocalizedName(block.getTranslationKey().substring(4));
    } else {
